...
The table tallies up the numbers of clumps of perfectly linked loci showing a particular significant relationship with fitness (rows) for various different $p$-value thresholds (columns). For example, 'Female early only' counts the number of loci whose expression significantly correlated with mean female early life fitness across lines. 'Age concordant, males' counts loci whose expression correlated with early- _and_ late-life fitness in males, in the same direction, while 'Age antagonistic, males' counts loci showing a significant, opposite relationships with male early- and male late-life fitness. Similarly, 'Sex concordant, early' counts loci showing a concordant effect on early life fitness in both males and females, and 'Sex antagonistic, late' counts those showing opposing effects on late-life fitness in males and females. The categories are all mutually exclusive, such that loci are only counted towards the most specific category that applies to them. Note that this method has low power to detect loci that correlate with two or more fitness metrics, because there are two or more opportunities to make a 'false negative' error, and the power is low for any given locus (this is why we use a range of p-values, including some low ones).
...
make_tally <- function(p_cutoff){
  GWAS_results <- tbl(db, "univariate_lmm_results") %>%
    ...
    rename(beta_FE = beta_female_early_raw, beta_FL = beta_female_late_raw,
           beta_ME = beta_male_early_raw, beta_ML = beta_male_late_raw,
           pval_FE = pvalue_female_early_raw, pval_FL = pvalue_female_late_raw,
           pval_ME = pvalue_male_early_raw, pval_ML = pvalue_male_late_raw) %>% 
