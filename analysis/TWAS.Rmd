...
sig_transcripts_table <- get_sig(TWAS_ED) %>%
  left_join(TWAS_mashr_results %>% select(FBID, contains("ED_LFSR")), by = "FBID") %>% # LFSR for each trait (like p-value)
  left_join(tbl(db, "genes") %>% collect(), by = "FBID") %>% # gene info for each
