...
Here, we perform a large number of simple linear regressions, and obtain the slope (beta or $\beta$) and its standard error from a regression of transcript $i$ on fitness trait $j$. The number of regression run was `r n_transcripts*4`, i.e. 4 fitness traits $\times$ `r n_transcripts` transcripts. This approach is often called 'TWAS'. 
...
<!-- ### Calculate the p-value for the `sex:expression` interaction term -->

<!-- ```{r} -->
<!-- get_one_transcript <- function(gene){ -->
<!--   data.frame(expression_data_both_sexes[[1]],  -->
<!--              as.data.frame(expression_data_both_sexes[[2]])) %>% -->
<!--     select(line, sex, !!gene) %>% -->
<!--     filter(line %in% predicted_line_means$line) %>% -->
<!--     group_by(line, sex) %>% -->
<!--     summarise_all(~ mean(.x)) %>% -->
<!--     left_join(predicted_line_means %>% select(-block), by = "line") %>% -->
<!--     mutate(early_fitness = female.fitness.early, -->
<!--            early_fitness = replace(early_fitness, sex == "male", male.fitness.early[sex == "male"]), -->
<!--            late_fitness = female.fitness.late, -->
<!--            late_fitness = replace(late_fitness, sex == "male", male.fitness.late[sex == "male"])) %>% -->
<!--     filter(!is.na(early_fitness)) %>% select(-contains("male")) -->
<!-- } -->

<!-- model_one_transcript <- function(focal_gene){ -->
<!--   dat <- get_one_transcript(focal_gene) -->
<!--   formula1 <- glue("early_fitness ~ {focal_gene} * sex") -->
<!--   formula2 <- glue("late_fitness ~ {focal_gene} * sex") -->
<!--   interaction_p_early <- summary(lm(formula1, data = dat))$coefficients[4,4] -->
<!--   interaction_p_late <- summary(lm(formula2, data = dat))$coefficients[4,4] -->
<!--   data.frame(focal_gene, interaction_p_early, interaction_p_late) -->
<!-- } -->

<!-- if(!file.exists("data/derived/TWAS/interaction_p_values.csv")){ -->
<!--   interaction_p_values <- colnames(expression_data_both_sexes[[2]]) %>%  -->
<!--     mclapply(model_one_transcript) %>% bind_rows() %>% as_tibble() -->
<!--   write_csv(interaction_p_values, "data/derived/TWAS/interaction_p_values.csv") -->
<!-- } else interaction_p_values <- read_csv("data/derived/TWAS/interaction_p_values.csv") -->
<!-- ``` -->
