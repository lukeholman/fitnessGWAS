...
This took about 4.5 days on my 8-core Mac. 
```{r}
# Make a fam file with all the 36,000+ gene expression traits (that's 18k per sex)
replace_pheno_data("all")
...
do_one_eQTL <- function(FBID){
  ...
  run_command("gemma -bfile dgrp2_QC_all_lines -lm 1 -n {female_index} -o eQTL_{FBID}_females", path = "/data/derived")
  run_command("gemma -bfile dgrp2_QC_all_lines -lm 1 -n {male_index}   -o eQTL_{FBID}_males",   path = "/data/derived")
  
  results_females <- read_tsv(glue("data/derived/output/eQTL_{FBID}_females.assoc.txt")) %>% 
    mutate(FBID = paste(FBID, "_females", sep = "")) 
  results_males <- read_tsv(glue("data/derived/output/eQTL_{FBID}_males.assoc.txt")) %>% 
  ...
  # Now get the results for these eQTLs, including the result for the non-significant sex
...
# system.time(do_one_eQTL("FBgn0000015")) # TEST
...
list.files("data/derived/eQTL_results") %>% 
...
```{r}
xx <- read_csv("data/derived/luke_eQTL_results.csv") %>% 
  arrange(p_wald) %>% filter(-log10(p_wald) > 8) 

xx <- xx %>%
  mutate(sex = ifelse(str_detect(FBID, "female"), "Female", "Male"),
         transcript = str_extract(xx$FBID, "FBgn[:digit:]+"),
         transcript = ifelse(is.na(transcript), str_extract(xx$FBID, "XLOC_[:digit:]+"), transcript)) 
library(ggplot2)

# Very imbalanced for females! I think: major alleles that depress female expression are rare... could be opposite way
xx %>% ggplot(aes(beta)) + geom_histogram(bins = 100) + facet_wrap(~sex)

xx <- xx %>%
  group_by(eQTL, sex) %>%
  summarise(mean_beta = mean(beta),
            FBID = FBID[1]) %>%
  mutate(chromosome = gsub("_", "", (substr(eQTL, 1, 2))),
         FBID = str_remove_all(FBID, "_female"),
         FBID = str_remove_all(FBID, "_male"),
         position = as.numeric(map_chr(strsplit(eQTL, split = "_"), ~ .x[2]))) 

xx %>%
  ggplot(aes(sex, mean_beta, group = eQTL)) + 
  geom_line(alpha = 0.1) + 
  facet_wrap(~chromosome)

xx %>%
  ggplot(aes(position, mean_beta, group = 1)) + 
  geom_line() +
  facet_wrap(chromosome~sex, ncol = 2, scales = "free_x")

xx %>%
  spread(sex, mean_beta) %>%
  filter(!is.na(Female) & !is.na(Male)) %>% 
  mutate(mean_sex = (Female + Male) / 2) %>%
  ggplot(aes(position, mean_sex, group = 1)) + 
  geom_line() +
...
xx %>%
  spread(sex, mean_beta) %>%
  filter(!is.na(Female) & !is.na(Male)) %>%
  filter((Female - 0.5) > Male) %>%
  left_join(tbl(db, "variants") %>% select(SNP, site.class, MAF) %>%
              collect(), by = c("eQTL" = "SNP")) %>%
  left_join(tbl(db, "genes") %>% select(FBID, gene_name) %>% collect(), by = "FBID") %>% print(n=Inf)
