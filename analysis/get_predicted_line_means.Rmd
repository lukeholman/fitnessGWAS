...
```{r, include=FALSE}
library(knitrhooks) # for scrollable output of the brms models below
output_max_height()
...
To explain, the terms with the form `(1 | x | y)` represent random intercepts for line, block, and vial ID (i.e. `y`), where the letter between the pipe operators (`x`) indicates which random intercepts are assumed to be correlated with which other random intercepts. Thus, we assume that the effects of line, block, and vial ID on early- and late-life fitness are potentially correlated, and estimate this correlation using the data and prior.
...
For both models, we assume the default priors except for the fact that we chose more informative, 'skeptical' priors for the random intercept terms, of the form `cauchy(0, 0.1)`. This prior helps the model to converge by making very large estimates implausible, constraining the parameter space. 
