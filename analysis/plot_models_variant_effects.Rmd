...
merged %>% filter(paste(chr, position) %in% paste(ruzicka_SA$chr, ruzicka_SA$position)) %>% pull(inter_sex_early) %>% hist()
merged %>% filter(!(paste(chr, position) %in% paste(ruzicka_SA$chr, ruzicka_SA$position))) %>% pull(inter_sex_early) %>% hist()

hist(-1 * (merged %>% filter(inter_sex_early <= quantile(inter_sex_early, 0.99)) %>% pull(ruzicka_effect_size) %>% log10))

bind_rows(merged %>% filter(inter_sex_early <= quantile(inter_sex_early, 0.01)) %>% mutate(type ="holmanSA"),
          merged %>% filter(inter_sex_early >= quantile(inter_sex_early, 0.99)) %>% mutate(type ="holmanSC")) %>% 
  ggplot(aes(type, -1 * log10(ruzicka_wald_p))) +
  geom_boxplot(outlier.shape = NA) + geom_beeswarm(alpha=0.3)

merged %>% filter(inter_sex_early <= quantile(inter_sex_early, 0.01) & -1 * log10(ruzicka_wald_p) >3)
tbl(db, "variants") %>% filter(SNP=="2L_18784363_SNP")
