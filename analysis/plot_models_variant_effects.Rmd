...
```{r manhattan, fig.height=12, fig.showtext=TRUE}
...
png("figures/fig2_SNPs_manhattan_plot.png", height = 8, width = 8, res = 300, units = "in")
...
pdf("figures/fig4_quartiles_plot.pdf", height = 6, width = 7)
grid_arrange_shared_legend(fig_4_top_row + 
                             theme(axis.text.x = element_blank(),
                                   axis.ticks.x = element_blank()) +
                             xlab(" "), 
                           fig_4_bottom_row, position = "right")
dev.off()
remove_pages(1, "figures/fig4_quartiles_plot.pdf", "figures/fig4_quartiles_plot.pdf")
```
...
```{r fig.showtext=T, fig.height=7, fig.width=7} 
# plotting models
...
## GO enrichment of top 1% sexually antagonistic transcripts

```{r}
library(GOfuncR) # BiocManager::install("GOfuncR")
library(org.Dm.eg.db) 

# This code labels the transcripts we analysed as either in or not-in the top 1%,
# and gets their gene symbols, setting up the data in the format required by GOfuncR
# as described in the vignette here https://bioconductor.org/packages/devel/bioc/vignettes/GOfuncR/inst/doc/GOfuncR.html
go_data <- antagonism_evidence_ratios_twas %>% 
  arrange(inter_sex_early) %>% 
  mutate(row = 1:n(),
         is_candidate = as.numeric(row < (n() / 100))) %>% 
  dplyr::select(FLYBASE = FBID, is_candidate) %>% 
  left_join(select(org.Dm.eg.db, keys = keys(org.Dm.eg.db), columns = c("SYMBOL","FLYBASE")), by = "FLYBASE") %>% 
  dplyr::select(SYMBOL, is_candidate) %>% 
  as.data.frame()

# Run a hypergeometric test onn the BP intology. This uses the software FUNC for the GO test:
# Pruefer, K. et al. (2007). FUNC: A package for detecting significant associations between gene sets and ontological annotations, BMC Bioinformatics 8: 41. [https://doi.org/10.1186/1471-2105-8-41]

res_hyper_fly <- go_enrich(go_data, orgDb='org.Dm.eg.db', test = "hyper", n_randsets = 10000,
                           silent = T, domains = "biological_process")
res_hyper_fly$results %>% 
  filter(raw_p_overrep < 0.01) %>% 
  dplyr::select(node_id, node_name, raw_p_overrep, FWER_overrep) %>% 
